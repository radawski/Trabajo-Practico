 ...
 */
public class PasajeroFileDAO implements PasajeroDAO {

    private final String rutaArchivo;

    public PasajeroFileDAO(String rutaArchivo) {
    ...
    public void modificar(Pasajero pasajero) {
        List<Pasajero> pasajeros = obtenerTodos();
        for (int i = 0; i < pasajeros.size(); i++) {
            Pasajero p = pasajeros.get(i);
            if (p.getTipoDocumento().equals(pasajero.getTipoDocumento()) && p.getNroDocumento().equalsIgnoreCase(pasajero.getNroDocumento())) {
                pasajeros.set(i, pasajero);
                break;
            }
        }
        guardarTodos(pasajeros);
    }

    public void eliminar(Pasajero pasajero) {
        List<Pasajero> pasajeros = obtenerTodos();
        pasajeros.removeIf(p -> p.getTipoDocumento().equals(pasajero.getTipoDocumento()) && p.getNroDocumento().equalsIgnoreCase(pasajero.getNroDocumento()));
        guardarTodos(pasajeros);
    }

    private void guardarTodos(List<Pasajero> pasajeros) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(rutaArchivo))) {
            ...
            throw new RuntimeException("Error al guardar pasajeros", e);
        }
    }

    public boolean esPasajeroRepetido(Pasajero pasajero) {
        return buscarPorDocumento(pasajero.getTipoDocumento(), pasajero.getNroDocumento()) != null;
    }
}
